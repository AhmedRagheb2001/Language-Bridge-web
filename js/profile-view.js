...
async function loadProfilePage(userId) {
  try {
    ...
    /* Watch presence for this user using global socket */
    watchUserPresence(userId);
    addPresenceCallback(presenceHandler);

    function presenceHandler(payload) {
      const id = payload?.userId || payload?.user_id || payload?.id;
      if (id == null || String(id) !== String(userId)) return;
...
function renderPublicProfile(profile, userId, relationship) {
  ...
  /* Default to unknown presence; will be updated via socket callback */
  const presClass = "offline";
  const presLabel = "Language partner";
