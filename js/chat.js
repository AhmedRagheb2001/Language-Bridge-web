...
async function openChat(id) {
  ...
  /*
   * Leave the previously active chat room (if any) before joining
   * the new one, so the server doesn't keep broadcasting this
   * socket into a conversation that's no longer open.
   */
  if (socket && socketConnected && window.activeChat && window.activeChat !== id) {
    socket.emit(SOCKET_EVENTS.leave, { chatId: window.activeChat });
  }
  ...
  /*
   * Join the Socket.IO room for this chat right away. Without this,
   * the socket only ever joined a room once, inside the "connect"
   * handler, at a point where window.activeChat was usually still
   * unset -- so live messages were never actually delivered.
   */
  if (socket && socketConnected) {
    socket.emit(SOCKET_EVENTS.join, { chatId: id });
  }
